...
def create_mega_table_intermediate():
  ...
  createTableQuery = r"""
  DROP TABLE IF EXISTS network.etm_mega_table_intermediate;
  CREATE TABLE network.etm_mega_table_intermediate
  (
    "A" integer,
    "B" integer,
    costs int[],
  ...
  logger.info("*" * 15)
  logger.info("Created ETM Mega Table Intermediate")
...
def populate_mega_table_intermediate():
  ...
  for i in etm_ranges:
    ...
    insert_query = 'INSERT INTO network.etm_mega_table_intermediate VALUES {0}'.format(args_str)
    cursor.execute(insert_query, listOfTuples)
    print str(i) + "ith insert done"
    conn.commit()
    
  logger.info("*" * 15)
  logger.info("Populated ETM Mega Table Intermediate")
...
def create_mega_table_final():
  ...
  createTableQuery = r"""
  DROP TABLE IF EXISTS network.etm_mega_table;
  CREATE TABLE network.etm_mega_table
  (
    "A" integer,
    "B" integer,
    "R1" numeric,
  ...
  logger.info("*" * 15)
  logger.info("Created ETM Final Mega Table")
...
def populate_mega_table_final():
  ...
  cursor.execute("SELECT * FROM network.etm_mega_table_intermediate;");
  ...
  logger.info("*" * 15)
  logger.info("Populated ETM Final Mega Table")
